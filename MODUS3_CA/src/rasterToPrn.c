...
static const struct command PresentationCommand [2] = {
    {4, (char[4]){0x1D, 'e', 0x03, 0xFF}},   // Hold in presentation
    {3, (char[3]){0x1D, 'e', 0x05}},         // Eject immediately
};

// continuous mode commands
static const struct command ContinuousModeCommand [2] = {
...
int main(int argc, char *argv[]) {
    ...
    // SET CONTINUOUS MODE
    if(settings.continuousMode > -1)
        SendCommand(ContinuousModeCommand[settings.continuousMode]);
    
    // SET PRINT QUALITY
    if(settings.printQuality > -1)
        SendCommand(PrintQuality[settings.printQuality]);

    // SET PRINT DENSITY
    if(settings.printDensity > -1 && settings.printDensity < 10) // 10 means Don't set
    ...
    // SEND PRESENTATION COMMAND (HOLD IN / EJECT)
    if(settings.presentation > -1)
        SendCommand(PresentationCommand[settings.presentation]);
