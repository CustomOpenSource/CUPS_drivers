...
static const struct command CutCommand [3] = {
    {8,(char[8]){0x1C, 0xC0, '3', 0x0A, 0x0A, 0x0A, 0x0A, 0x0A}},   // tear off
    {3,(char[8]){0x1C, 0xC0, '4'}},                                 // cut and recovery
    {3,(char[8]){0x1C, 0xC0, '5'}}                                  // cut without recovery
};
...
int main(int argc, char *argv[]) {
    ...
    while (cupsRasterReadHeader2(rasterFileStruct, &cupsHeader)){
        ...
        // SEND CUT COMMAND
	    if (settings.cutterMode == 2 || settings.cutterMode == 3) {
		    SendCommand(CutCommand[settings.cutterMode - 1]);
		}
    ...
    // SEND CUT COMMAND
	if (settings.cutterMode == 0 || settings.cutterMode == 4 || settings.cutterMode == 5) {
		SendCommand(CutCommand[settings.cutterMode == 0 ? 0 : settings.cutterMode - 3]);
	}
