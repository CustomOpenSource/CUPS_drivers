...
// eject command 
static const struct command EjectBehaviorCommand[8] = {
    {4, (char[4]){0x1C, 'P', 0x01, 0x10}},  // hold
    {4, (char[4]){0x1C, 'P', 0x00, 0x00}},  // eject
    {4, (char[4]){0x1C, 'P', 0x01, 0x05}},  // hold 5s
    {4, (char[4]){0x1C, 'P', 0x01, 0x0A}},  // hold 10s
    {4, (char[4]){0x1C, 'P', 0x01, 0x14}},  // hold 20s
    {4, (char[4]){0x1C, 'P', 0x01, 0x1E}},  // hold 30s
    {4, (char[4]){0x1C, 'P', 0x01, 0x3C}},  // hold 60s
    {4, (char[4]){0x1C, 'P', 0x01, 0x78}}   // hold 120s
};
...
void initializeSettings(char * commandLineOptionSettings, struct PrinterSetUp * settings){
    ...
    settings->ejectorBehavior = FindOption("EjectorBehavior", ppd);
	...
	syslog(LOG_NOTICE,"settings->ejectorBehavior = %d\n",settings->ejectorBehavior);
#endif
...
int main(int argc, char *argv[]) {
    ...
    while (cupsRasterReadHeader2(rasterFileStruct, &cupsHeader)){
        ...
        // ALIGN TO NOTCH
        if( settings.notchAlignment > 0)
            SendCommand(NotchAlignmentCommand[1]);

        if( settings.ejectorBehavior >= 0 )
            SendCommand(EjectBehaviorCommand[settings.ejectorBehavior]);
