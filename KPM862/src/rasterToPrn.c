...
static const struct command DarknessCommand [5] = {
...
static const struct command CutterModeCommand = {
    2, (char[2]){0x1B, 'i'},		// Enable 
};


// document resolution
static const struct command DocumentResolutionCommand = {
    7, (char[7]){0x1B,'*','t','2','0','0','R'}
};

// compression commands
static const struct command CompressionCommand [2] = {
...
// begin page command
static const struct command BeginPageCommand = {
    3, (char[3]){0x18, 0x1B, '2'} 
};
...
static const struct command EndDocCommand = {
    4, (char[11]){0x1D, 'P', 0x00, 0x00}   
};

// ignore text: prevents the printer from mistakenly printing text during a graphic print
static const struct command IgnoreTextCommand [2] = {
...
static const struct command PrintRotationCommand [3] = {
    {3, (char[5]){0x1B, 0x7B, 0x00}},   // normal
    {3, (char[5]){0x1B, 0x7B, 0x01}}    // 180 degres
};
...
void initializeSettings(char * commandLineOptionSettings, struct PrinterSetUp * settings){
    ...
    settings->printRotation = FindOption("PrintRotation", ppd);
	...
	syslog(LOG_NOTICE,"settings->printRotation = %d\n",settings->printRotation);
#endif
...
int main(int argc, char *argv[]) {
    ...
    // IGNORE TEXT -> ON
    SendCommand(IgnoreTextCommand[0]);

    // SET ROTATION
    if( settings.printRotation > 0 )
        SendCommand(PrintRotationCommand[settings.printRotation]);
    ...
    // SET PRINT DENSITY
    if( settings.darkness > -1 && settings.darkness < 5 ) // 5 means Don't set
    ...
    while (cupsRasterReadHeader2(rasterFileStruct, &cupsHeader)){
    	...
    	if( strstr(cupsHeader.cupsPageSizeName, (const char*)"Roll") ){
            ...
            #ifdef DEBUG_SYSLOG
                syslog(LOG_NOTICE,"Selected a \"Roll\" page");
    	...
    	SendCommand(DocumentResolutionCommand);
        ...
        SendCommand(CompressionCommand[0]);
        ...
        for (int y = 0; y < cupsHeader.cupsHeight; y ++){
    	    ...
    	    if (lineIsWhite == 0){
                ...
                SendWithCompression(rasterLine, bytesPerLine, CompressionCommand[0]);
            }

        }

	    // END COMPRESSION MODE COMMAND
    	SendCommand(CompressionCommand[1]);

        // END PAGE COMMAND
        SendCommand(EndPageCommand);
        
        // ALIGN TO NOTCH
        if ( settings.notchAlignment == 1 || settings.notchAlignment == 2 )
    ...
    // SEND END DOCUMENT
    SendCommand(EndDocCommand);

    // IGNORE TEXT -> OFF
    SendCommand(IgnoreTextCommand[1]);
